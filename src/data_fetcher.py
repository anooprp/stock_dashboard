...
def fetch_stock_data1(tickers, period="1y"):
    ...
    stock_data, fundamentals, predictions, future_predictions = fetch_data_from_db(
        tickers, period
    )
    ...
    if fundamentals:
        ...
        start_time = time.perf_counter()
        predictions, future_predictions = predict_stock_prices_lstm(
            model, stock_data[tickers], scaler
        )
...
def fetch_stock_data(tickers, period="1y"):
    ...
    if fundamentals:
        ...
        print("Generating Data using API's")
        start_time = time.perf_counter()
        stock_data = fetch_yahoo_data(
            [tickers], period=period
        )  # Pass tickers directly (not as a list of a list)
        timings["fetch_yahoo_data"] = time.perf_counter() - start_time
        ...
        X, y, scaler = prepare_lstm_data(
            stock_data[tickers]
        )  # Use stock_data[tickers] to ensure proper indexing
        timings["prepare_lstm_data"] = time.perf_counter() - start_time
        ...
        model = build_cnn_lstm_model(
            (X.shape[1], X.shape[2])
        )  # Fix the input shape for Conv1D with multiple features
        timings["build_cnn_lstm_model"] = time.perf_counter() - start_time
