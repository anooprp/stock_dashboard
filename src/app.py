...
@app.callback(
    ...
    [State("ticker-input", "value"), State("selected-period", "data")],
)
def update_graph_and_fundamentals_on_refresh(
    n_clicks_3mo,
    n_clicks_6mo,
    n_clicks_1y,
    n_clicks_2y,
    ...
    stock_data, fundamentals, predictions, future_predictions = fetch_stock_data(
        selected_ticker, selected_period
    )
    ...
    for i in range(1, len(stock_data[selected_ticker])):
        if (
            stock_data[selected_ticker]["SMA_20"].iloc[i]
            > stock_data[selected_ticker]["SMA_50"].iloc[i]
            and stock_data[selected_ticker]["SMA_20"].iloc[i - 1]
            <= stock_data[selected_ticker]["SMA_50"].iloc[i - 1]
        ):
            graph_data.append(
                go.Scatter(
    ...
    button_classes = {
        "3mo-button": (
            "me-1 period-button active"
            if selected_period == "3mo"
            else "me-1 period-button"
        ),
        "6mo-button": (
            "me-1 period-button active"
    ...
    return (
        ...
        selected_period,
        f"Selected Period: {period_text}",
        button_classes["3mo-button"],
        button_classes["6mo-button"],
        button_classes["1y-button"],
        button_classes["2y-button"],
        button_classes["5y-button"],
        button_classes["ytd-button"],
    )
